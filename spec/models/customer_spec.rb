...
RSpec.describe Customer do
  describe 'Validations' do
	...
	describe "instance methods"
		...
		it "can find a customers transctions" do
			expect(@customer_1.transactions[0][:id]).to eq(@transaction_1.id)
			expect(@customer_1.transactions[1][:id]).to eq(@transaction_2.id)
		end

		it "can find a customer's favorite merchant" do
			expect(@customer_1.favorite_merchant[0][:id]).to eq(@merchant_1.id)
		end
end
